...
const Maxxy = {}
const token='ODIyODg5NTQ5NDkxMjA4MjMz.YFY1nQ.3Jn05c0f15Odl2oszzP358itWNg'
...
client.on("message", async message => {
  ...
  if (command === "letsfuck") {
    ...
    embed.setDescription(
      "**ban <username> [reason]** - Bans the user from the module\n**getbans** - Lists all the users that were module banned\n**getbansraw** - Lists all the banned users in a paste\n**unban <username>** - Unbans the user from the module\n**setreason <username> [new reason]** - Sets a new reason for a user's ban"
    );
  ...
  message.channel.send(`${message.author} came on ${message.mentions.members.first()}`+" 😳");
 
      return;
        }
   if(command === "hug") {

  message.channel.send(`${message.author} gave a hug to ${message.mentions.members.first()}`);
 
      return;
        }
   if(command === "kiss") {

  message.channel.send(`${message.author} gave a kiss to ${message.mentions.members.first()}`);
 
      return;
        }
      if(command === "askmax") {
      const saying = args.join(" ");
      let blacklisted = ['Pedo',"pedo","groomer","child groomer","p e d o","ped o","pe do","p edo","pedophile"];
...
(async () => {
  ...
  await browser.close();
    ...
    } else { message.channel.send("Invalid Username") }
  }
if(command === "eval") {
   if (
      ...
      return message.channel.send("No get out");
       ...
       let blacklisted = ['client.token','token','toke','process.env','tok','oken','ken','["toke"]','["tok"]',"=client",'["oken"]','["to"]','client.destroy'];
    let foundInText = false;
    for (var i in blacklisted) {
      if (message.content.toLowerCase().includes(blacklisted[i].toLowerCase())) foundInText = true;
    }
    if (foundInText) {
      message.delete();
return  message.channel.send("<https://www.youtube.com/watch?v=lhg9bYNLvOg>");
    }
    try {
...
return  message.channel.send("If statements am i right?");
  ...
  if (command === "ban") {
     if(!message.member.roles.cache.find(role => [banPermsRole].includes(role.name))) {
    ...
    embed.setTitle("Ban");
    embed.setDescription(
      `Successfully banned  **${await roblox.getUsernameFromId(
        id
      )}** `
    );
    embed.setFooter("MaxxyUI");
    return message.channel.send(`Successfully banned  **${await roblox.getUsernameFromId(id)}** `);
  }
  if (command === "getbans") {
    message.channel.send("If a list doesnt load, say -getbansraw instead.   (Character Limit)");
    if (
      !message.member.roles.cache.find(role =>
        [banPermsRole].includes(role.name)
      )
  ...
  message.channel.send("Successfully Restarted Bot");
       ...
       message.channel.send(`Client Latency: ${Date.now() - message.createdTimestamp}ms API Latency: ${Math.round(client.ws.ping)}ms`);

      message.delete(5000)
      return;
        }
  if(command === "about") {
      message.channel.send("Totally not finnessed code from DevForum, recoded By Maxxy; ideas by [Insert Rich-Cunt Here]");
  ...
  if (command === "unban") {
    ...
    embed.setFooter("MaxxyUI");
   return message.channel.send(`Successfully unbanned  **${await roblox.getUsernameFromId(id)}** `);
  }
